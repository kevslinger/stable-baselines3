...
class SubgoalHerReplayBuffer(DictReplayBuffer):
    """
    ...
    def __init__(
        ...
        super(SubgoalHerReplayBuffer, self).__init__(buffer_size, env.observation_space, env.action_space, device, env.num_envs)
    ...
    def sample_subgoals(self, episode_indices, her_indices, transitions_indices):
        # Provide a subgoal which will push the agent towards where it needs to go.
        # If we're in or outside the first occluded region, give a goal that is not occluded and close.
        new_goals = []
        tmp_env = self.env

        # self.env = self.env.unwrapped
        # print(self.env)
        # print(self.env.unwrapped)
        # print(self.env.envs[0])
        # print(self.env.envs[0].unwrapped)
        self.env = self.env.envs[0].unwrapped
        # This should give us a list of all the observations we're sampling
        #print(episode_indices[her_indices].shape)
        #print(transitions_indices.shape)
        for obs in self._buffer["achieved_goal"][episode_indices[her_indices], transitions_indices]:
            obs = obs[0]
            # handle the case where the agent is either in or outside the outer occluded region
            # Too far left
            if obs[0] <= 0.38:
                # Also too far up
                if obs[1] <= -0.2:
                    new_goals.append(np.array([0.38, -0.2]))
                # Also too far down
                elif obs[1] >= 0.2:
                    new_goals.append(np.array([0.38, 0.2]))
                # In the middle
                else:
                    new_goals.append(np.array([0.38, obs[1]]))
            # Too far right
            elif obs[0] >= 0.58:
                # Also too far up
                if obs[1] <= -0.2:
                    new_goals.append(np.array([0.58, -0.2]))
                # Also too far down
                elif obs[1] >= 0.2:
                    new_goals.append(np.array([0.38, 0.2]))
                # In the middle
                else:
                    new_goals.append(np.array([0.58, obs[1]]))
            # Too far up
            elif obs[1] <= -0.2:
                new_goals.append(np.array([obs[0], -0.2]))
            # Too far down
            elif obs[1] >= 0.2:
                new_goals.append(np.array([obs[0], 0.2]))
            # Inside inner occluded region (don't change goal)
            elif self.env.in_inner_occluded_region(obs[0], obs[1]):
                new_goals.append(obs)
            # In between the occluded regions
            else:
                # Too far left
                if obs[0] <= self.env.inner_occluded_region_x_left:
                    # Too far down
                    if obs[1] <= self.env.inner_occluded_region_y_down:
                        new_goals.append(np.array([self.env.inner_occluded_region_x_left, self.env.inner_occluded_region_y_down]))
                    # Too far up
                    elif obs[1] >= self.env.inner_occluded_region_y_up:
                        new_goals.append(np.array([self.env.inner_occluded_region_x_left, self.env.inner_occluded_region_y_up]))
                    else:
                        new_goals.append(np.array([self.env.inner_occluded_region_x_left, obs[1]]))
                # Too far right
                elif obs[0] >= self.env.inner_occluded_region_x_right:
                    # Too far down
                    if obs[1] <= self.env.inner_occluded_region_y_down:
                        new_goals.append(np.array([self.env.inner_occluded_region_x_right, self.env.inner_occluded_region_y_down]))
                    # Too far up
                    elif obs[1] >= self.env.inner_occluded_region_y_up:
                        new_goals.append(np.array([self.env.inner_occluded_region_x_right, self.env.inner_occluded_region_y_up]))
                    else:
                        new_goals.append(np.array([self.env.inner_occluded_region_x_right, obs[1]]))
                # Too far down
                elif obs[1] <= self.env.inner_occluded_region_y_down:
                    new_goals.append(np.array([obs[0], self.env.inner_occluded_region_y_down]))
                # Too far up
                elif obs[1] >= self.env.inner_occluded_region_y_up:
                    new_goals.append(np.array([obs[0], self.env.inner_occluded_region_y_up]))
                # I don't think there can be an else case here...
                else:
                    print("SOMETHING HAS GONE HORRIBLY WRONG")
                    exit(0)
            new_goals[-1] = np.expand_dims(new_goals[-1], axis=0)
        self.env = tmp_env
        return new_goals
    ...
    def _sample_transitions(
        ...
        # print(her_indices)
        # print(transitions_indices)
        # print(episode_indices[her_indices])
        # print(transitions_indices[her_indices])
        # print(f"Ep indices shape: {episode_indices.shape}")
        # print(f"her indices shape: {her_indices.shape}")
        # print(f"transition indices shape: {transitions_indices.shape}")
        # print(f"Transitions indices [her_indices] shape: {transitions_indices[her_indices].shape}")
        new_goals = self.sample_subgoals(episode_indices, her_indices, transitions_indices[her_indices])
        #print(self.pos * self.max_episode_length)
        # if self.pos * self.max_episode_length > 10:
        #     print(f"Episode indices: {episode_indices}")
        #     print()
        #     print(f"her indices: {her_indices}")
        #     print()
