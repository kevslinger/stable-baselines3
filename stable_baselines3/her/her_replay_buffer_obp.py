...
class OcclusionPrioritizedHerReplayBuffer(HerReplayBuffer):
    """
    ...
    def __init__(
        ...
        super(OcclusionPrioritizedHerReplayBuffer, self).__init__(env, buffer_size, device, replay_buffer, max_episode_length,
                                              n_sampled_goal, goal_selection_strategy, online_sampling, handle_timeout_termination)
    ...
    # TODO
    # OCCLUSION BASED PRIORITIZATION
    # What I really want to do is
    # np.random.choice(range(self.pos),
    #                  size=batch_size,
    #                  replace=True,
    #                  p=np.count_nonzero(self.observations['achieved_goal']==np.zeros(len(self.observations['achieved_goal'][0]))))
    # I want to either sample transitions which have the most occlusions in them, or sample from trajectories which have
    # the most occlusions
    def _sample_transitions(
        ...
        #     print(f"Printing occlusions")
        #     print([occlusions for occlusions in self._buffer['occlusions']])
        #     episode_indices = (
        #         np.random.choice(range(1, self.n_episodes_stored), size=batch_size, p=[occlusions/sum(self._buffer['occlusions']) for occlusions in self._buffer['occlusions']])
        #     )
        # else:
